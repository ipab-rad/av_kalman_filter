...
namespace util
{
    TwistMsgConverter::TwistMsgConverter(const rclcpp::NodeOptions &options) : Node("twist_msg_convertor", options)
    {
        // These standard deviation values are empiracally estimated
        // linear_vel_std_ = 0.1;  
        // angular_vel_std_ = 0.01;
        ...
        twist_sub_ = this->create_subscription<TwistStamped>(
            "twist_in", 10, std::bind(&TwistMsgConverter::imu_callback, this,  std::placeholders::_1));

        twist_pub_ = this->create_publisher<TwistWithCovarianceStamped>("twist_out", 10);
    ...
    void TwistMsgConverter::imu_callback(const TwistStamped & msg)
    {
        // Convert to TwistWithCovarianceSTamped

        TwistWithCovarianceStamped new_msg;
